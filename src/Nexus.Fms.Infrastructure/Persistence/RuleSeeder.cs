...
public static class RuleSeeder
{
    ...
    {
        FraudRule R(string code, string name, RuleCategory cat, int score, string json,
        ...
        return new[]
        {
            R("R01", "Amount exceeds customer 95th percentile", RuleCategory.Amount, 40,
                """{ "fact": "amountToP95Ratio", "op": "Gt", "value": 2 }"""),

            R("R02", "Velocity: >5 transfers in 10 minutes", RuleCategory.Velocity, 50,
                """{ "fact": "outboundTransfersLast10Min", "op": "Gt", "value": 5 }"""),

            R("R03", "Beneficiary on NIBSS fraud watchlist", RuleCategory.Watchlist, 50,
                """{ "all": [ { "fact": "receiverOnNibssWatchlist", "op": "IsTrue" }, { "not": { "fact": "receiverNibssConfirmedFraud", "op": "IsTrue" } } ] }"""),

            R("R03B", "Beneficiary is NIBSS confirmed fraud", RuleCategory.Watchlist, 100,
                """{ "fact": "receiverNibssConfirmedFraud", "op": "IsTrue" }""", noOffset: true),

            R("R04", "First-time beneficiary + amount > N500K", RuleCategory.Beneficiary, 40,
                """{ "all": [ { "fact": "isFirstTimeBeneficiary", "op": "IsTrue" }, { "fact": "amount", "op": "Gt", "value": 500000 } ] }"""),

            R("R05", "New device + transfer within 30 min of first login", RuleCategory.Device, 30,
                """{ "all": [ { "fact": "isNewDevice", "op": "IsTrue" }, { "fact": "minutesSinceFirstLogin", "op": "Lt", "value": 30 } ] }"""),

            R("R06", "Transaction outside 11PM-5AM", RuleCategory.Time, 15,
                """{ "any": [ { "fact": "hourOfDay", "op": "Gte", "value": 23 }, { "fact": "hourOfDay", "op": "Lt", "value": 5 } ] }"""),

            R("R07", "Multiple failed OTP then successful transfer", RuleCategory.Auth, 60,
                """{ "fact": "failedOtpAttemptsLast15Min", "op": "Gte", "value": 3 }"""),

            R("R08", "Cumulative daily outflow > 80% of balance", RuleCategory.Balance, 35,
                """{ "fact": "outflowPercentOfBalance", "op": "Gt", "value": 80 }"""),

            R("R09", "Sender on internal blacklist (confirmed fraud)", RuleCategory.Blacklist, 100,
                """{ "fact": "senderOnInternalBlacklist", "op": "IsTrue" }""", noOffset: true),

            R("R10", "Inbound credit from watchlisted sender", RuleCategory.Inbound, 30,
                """{ "all": [ { "fact": "direction", "op": "Eq", "value": "Inflow" }, { "fact": "senderOnNibssWatchlist", "op": "IsTrue" } ] }"""),

            R("R11", "Structured deposits (smurfing pattern)", RuleCategory.Inbound, 25,
                """{ "fact": "inboundCreditsSimilarAmountLast24h", "op": "Gte", "value": 3 }""", sync: false),

            R("R12", "Round-number transfers >3x in 24hrs", RuleCategory.Pattern, 10,
                """{ "fact": "roundNumberOutflowsLast24h", "op": "Gt", "value": 3 }""", sync: false),

            R("R13", "Customer is whitelisted", RuleCategory.Mitigant, -10,
                """{ "fact": "isWhitelisted", "op": "IsTrue" }"""),

            R("R14", "Transaction to previously successful beneficiary", RuleCategory.Mitigant, -5,
                """{ "fact": "successfulTransfersToBeneficiary", "op": "Gte", "value": 3 }"""),

            R("R15", "NIBSS lookup unavailable", RuleCategory.Compensating, 5,
